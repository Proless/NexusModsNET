...
namespace NexusModsNET
{
	...
	/// </summary>
	public class NexusModsAPIClient : INexusModsAPIClient, IDisposable
	{
		#region Fields
		/// <summary>
		...
		#region Constructors
		/// <summary>
		/// A client for the NexusMods API, which requires at least a API key to be provided.
		/// </summary>
		...
		public NexusModsAPIClient(string apiKey, string productName, string productVersion)
		{
		...
		{
			T output = default;
			var httpRequestMessage = ConstructHttpRequestMessage(method, requestURI, formData);
		...
		/// <returns></returns>
		public HttpRequestMessage ConstructHttpRequestMessage(HttpMethod method, Uri requestURI, HttpContent httpContent = null, string acceptedMediaType = null)
		{
		...
		private void InitializeHttpClient()
		{
			...
			{
				BaseAddress = new Uri(Routes.BaseAPIURL)
			};
